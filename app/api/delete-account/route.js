...
export async function POST(req) {
    ...
    try {
        ...
        await pool.query('DELETE FROM taskreport_staff WHERE userId = ?', [userId]);
        await pool.query('DELETE FROM taskstaff WHERE userId = ?', [userId]);
        await pool.query('DELETE FROM taskreports WHERE reportUserId = ?', [userId]);
        await pool.query('DELETE FROM staff WHERE userId = ?', [userId]);
